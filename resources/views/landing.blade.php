...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .road-h { position:absolute; height:2px; background:rgba(255,255,255,0.06); border-radius:1px; }
        .road-v { position:absolute; width:2px;  background:rgba(255,255,255,0.06); border-radius:1px; }
        ...
        @keyframes vehicle-move {
            ...
            50%  { transform: translate(140px, 20px) rotate(120deg); }
            75%  { transform: translate(60px, 80px)  rotate(60deg); }
            100% { transform: translate(0px, 0px)    rotate(45deg); }
        }
        @keyframes vehicle2-move {
            0%   { transform: translate(0px, 0px)     rotate(200deg); }
            33%  { transform: translate(-60px, 50px)  rotate(240deg); }
            66%  { transform: translate(-100px,-30px) rotate(180deg); }
            100% { transform: translate(0px, 0px)     rotate(200deg); }
        }
        .vehicle-1 { animation: vehicle-move 12s ease-in-out infinite; }
        .vehicle-2 { animation: vehicle2-move 16s ease-in-out infinite; }
        .pulse-ring {
        ...
        .hero-content { animation: hero-in 0.8s ease-out 0.2s both; }
        .hero-visual  { animation: hero-in 0.8s ease-out 0.5s both; }
        ...
        .alert-badge { animation: alert-slide 4s ease-in-out infinite; }
        ...
        @media (prefers-reduced-motion: reduce) {
            *, .vehicle-1, .vehicle-2, .pulse-ring, .hero-content, .hero-visual, .reveal {
                    ...
                    @foreach([
                        ...
                        <div class="text-center reveal" style="transition-delay: {{ $i * 100 }}ms">
                            <div class="w-16 h-16 bg-[#021F4A] rounded-2xl flex items-center justify-center mx-auto mb-5 relative">
                                ...
                                @if($i < 2)
                                    <div class="absolute -right-4 top-1/2 -translate-y-1/2 text-gray-300 hidden md:block">→</div>
                                @endif
                            </div>
