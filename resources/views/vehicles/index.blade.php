...
<script>
    ...
    function openAddModal() {
        showEl('add-modal');
        document.getElementById('add-vehicleNumber')?.focus();
    }

    function closeAddModal() {
        hideEl('add-modal');
        hideErr('add-error');
        ['add-vehicleNumber', 'add-make', 'add-model', 'add-year', 'add-color'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
    ...
    async function submitEditVehicle() {
        ...
        const {
            ok,
            data
        } = await apiFetch(`/vehicles/${id}`, 'PUT', {
            vehicleNumber,
            make,
            model,
            year: parseInt(year),
    ...
    async function submitLinkDevice() {
        ...
        const {
            ok,
            data
        } = await apiFetch(`/vehicles/${vehicleId}/link-device`, 'POST', {
            imei
        });
    ...
    // ── UNLINK GPS ───────────────────────────────────────────────────────────
    async function openUnlinkModal(vehicleId, vehicleName) {
        ...
        try {
            ...
            const assignmentId = json?.data?.currentAssignmentId ?? json?.currentAssignmentId ?? null;
            if (!assignmentId) {
                alert('Could not find assignment ID. Please refresh.');
                return;
            }
            const {
                ok,
                data
            } = await apiFetch(`/vehicles/${vehicleId}/unlink-device`, 'POST', {
                assignmentId
            });
            if (ok) {
                window.location.reload();
            } else {
                alert(data.message ?? 'Failed to unlink device.');
            }
        } catch {
    ...
    document.addEventListener('keydown', e => {
        ...
        ['add-modal', 'edit-modal', 'link-modal', 'delete-modal'].forEach(hideEl);
    });
</script>
