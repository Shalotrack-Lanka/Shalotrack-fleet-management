...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden" id="complaint-root"
    ...
    <div class="px-6 py-4 border-t border-gray-100">
        ...
        <div class="flex gap-3">
            <input type="text" id="reply-message" maxlength="2000" placeholder="Type a reply…"
...
<script>
    ...
    // Guard: complaintId must look like a UUID to prevent URL injection
    ...
    async function submitReply() {
        hideErr('reply-error');
        if (!SAFE_ID.test(complaintId)) return;
        const input = document.getElementById('reply-message');
        const message = input?.value.trim();
        if (!message) {
            showErr('reply-error', 'Please type a message.');
            return;
        }
        ...
        const {
            ok,
            data
        } = await apiFetch(`/complaints/${complaintId}/reply`, 'POST', {
            message
        });
        ...
        if (ok) {
            ...
            showErr('reply-error', data.message ?? 'Failed to send reply.');
        }
    }
    ...
    // Foreground-only poll: reload only when reply count actually changes.
    // On 401, stop the interval immediately and redirect — don't leak timers.
    let pollInterval = null;
    if (complaintId && SAFE_ID.test(complaintId)) {
        pollInterval = setInterval(async () => {
            ...
            const {
                ...
                status,
                data
            } = await apiFetch(`/complaints/${complaintId}`, 'GET').catch(() => ({
                ok: false,
                status: 0,
                data: {}
            }));
