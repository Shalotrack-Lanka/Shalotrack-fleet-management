...
<script>
    ...
    function openFileModal() { showEl('file-modal'); }
    function closeFileModal() {
        hideEl('file-modal'); hideErr('file-error');
    ...
    async function submitComplaint() {
        ...
        if (!vehicleId)   { showErr('file-error', 'Please select a vehicle.'); return; }
        if (!description) { showErr('file-error', 'Please describe the issue.'); return; }

        setBtn('file-btn', true, 'Submit');
        const { ok, data } = await apiFetch('/complaints', 'POST', { vehicleId, category, description });
        setBtn('file-btn', false, 'Submit');

        if (ok) { closeFileModal(); window.location.reload(); }
        else    { showErr('file-error', data.message ?? 'Failed to submit complaint.'); }
    }
</script>
@endpush